...
int32_t	MS5611::getPressure(){
	...
	int64_t OFF  = (int64_t)_calibration[2-1]*65536 
				 + (int64_t)_calibration[4-1]*_dT/128;
	
	int64_t SENS = (int64_t)_calibration[1-1]*32768 
				 + (int64_t)_calibration[3-1]*_dT/256;
...
int32_t MS5611::getTemperature(){
	...
	_dT = D2-((uint32_t)_calibration[5-1] * 256); 		//update '_dT'
	...
	_T = 2000 + ((int64_t)_dT * _calibration[6-1])/8388608;
	return _T;
}
...
void MS5611::readCalibration(){
	for(uint8_t k=0;k<6;k++){
		...
		_calibration[k] = (uint16_t) (readnBytes(NBYTES_PROM) & 0xFFFF); //masking out two LSB
	}
}
